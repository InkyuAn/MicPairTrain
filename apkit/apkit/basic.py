...
def load_wav(filename, offset=0, nsamples=-1, ch_first=True):
    ...
    w.close()
    data = data.T if ch_first else data
    return fs, data
...
def stft(signal, win_size, hop_size, window=None, last_sample=False):
    ...
    if window is None:
        return np.array([[np.fft.fft(c[t:t + win_size])
